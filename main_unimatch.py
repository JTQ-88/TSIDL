...
from loader import get_dataloaders, get_dataloaders_unimatch
import metric
...
def train(model,
    ...
    model.train()
    train_bar = tqdm(zip(cycle(labeled_train_loader), unlabeled_train_loader))
    for batch_idx, data in enumerate(train_bar):
        x_l, labels_l = data[0][0], data[0][1]
        x_u, x_u_s1, x_u_s2, labels_u = data[1][0], data[1][1], data[1][2], data[1][3]
        x_l = x_l.cuda()
        x_u = x_u.cuda()
        x_u_s1 = x_u_s1.cuda()
        x_u_s2 = x_u_s2.cuda()
        ...
        # else:
        self_training_loss1 = (F.cross_entropy(y_u_s1, pseudo_labels, reduction='none') * mask).mean()
        self_training_loss2 = (F.cross_entropy(y_u_s2, pseudo_labels, reduction='none') * mask).mean()
        self_training_loss3 = (F.cross_entropy(y_u_fp, pseudo_labels, reduction='none') * mask).mean()
        self_training_loss = self_training_loss1 * 0.4 + self_training_loss2 * 0.4 + self_training_loss3 * 0.2
        # if batch_idx == 100:
        #     print(t_p)
        ...
        consistency_weight = get_current_consistency_weight(epoch, 0.2, 5)
        loss = cls_loss + self_training_loss * consistency_weight
        ...
        train_bar.set_description("CLS_Loss: %.4f, Semi_Loss: %.4f" % (
            cls_losses.avg,
            self_training_losses.avg
        ))
...
def test(model, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            logits, _ = model(data, infer=True)
...
def main():
    # test accuracy（OA）: 93.49  AA : 93.32  Kappa : 91.46
    dataset_names = ['KSC' 'PaviaU', 'Houston']
    ...
    parser.add_argument('--model', default='Network_Unimatch', type=str, choices=['Network_Unimatch', 'Network_Unimatch_Improve', 'Network'])
    ...
    parser.add_argument('--epoches', default=20, type=int, help='Number of sweeps over the dataset to train')
    ...
    for n in range(0, args.runs):
        ...
        labeled_train_loader, _, unlabeled_train_loader, _, _, _, unlabeled_dataset = get_dataloaders_unimatch(batchsize=batch_size, n=n, dataset=args.dataset)
        _, test_loader, _, TestLabel, TestPatch, pad_width_data, _ = get_dataloaders_unimatch(batchsize=batch_size, n=n, dataset=args.dataset)
        ...
        for epoch in range(1, epochs+1):
            ...
            loss_x, loss_u, selected_label = \
                train(model=model, epoch=epoch, optim=optimizer, lr_schdlr=lr_scheduler, classwise_acc=classwise_acc,
            ...
            if test_acc > best_acc:
                best_acc = test_acc
                best_epoch = epoch
                torch.save(model, 'results_unimatch/best_acc_result1.pth')
        print('Best test_acc_1: {:.2f} at epoch {}'.format(best_acc, best_epoch))

        model = torch.load('results_unimatch/best_acc_result1.pth')
        model.eval()
